...
def WashData(data):
    ...
    for idx,row in data.iterrows():
      ...
      # 第1户（父母家，1m网格情况）
        ...
        #     del_list.append(idx)
      # 第1户（父母家，0.5m网格情况）
        if (row['label']==325)or(row['label']==454):
            del_list.append(idx)
      # 第2户（舅舅家，1m网格情况）
        # if (row['label']==55):
        #     del_list.append(idx)
...
def createHeatmapByTime(dict_zones,figName,map_img,res_path,row_num,column_num,unit,max_x,min_x,max_y,min_y,groups):
    ...
    ax = sns.heatmap(new_grids, cmap='Blues',linewidths=0,alpha=0.8,xticklabels =False,square = True,annot=zone_grids,yticklabels =False,mask=(new_grids==0.),center=0.5)
    ax.invert_yaxis()
    #plt.show()
    plt.imshow(map_img,zorder = 0, extent=[-0.1,(max_x-min_x)/unit-0.2,-0.1,(max_y-min_y)/unit-0.3] ) # extent=[x_left/unit,x_right/unit,y_down/unit,y_up/unit])
...
def createHeatmapByTime2(new_grids,figName,map_img,res_path,unit,max_x,min_x,max_y,min_y):
    fig, ax = plt.subplots(figsize=(max_x-min_x,max_y-min_y))
    ax = sns.heatmap(new_grids, cmap='Blues',linewidths=0,alpha=0.8,xticklabels =False,square = True,annot=False,yticklabels =False,mask=(new_grids==0.),center=0.5)
    ax.invert_yaxis()
    #plt.show()
