...
class Family:
    def __init__(self,name,size):
        self.name=name
        self.size=size # in order as lx,ux,ly,uy
        self.scenario=[]
...
def parseFamily(xmlFile):
    if os.path.isfile(xmlFile):
        ...
        for f in root.findall('family'):
            sz = f.find('size')
            size = [float(sz.get('xlow')),float(sz.get('xup')),float(sz.get('ylow')),float(sz.get('yup'))]
            family = Family(f.get('name'),size)
...
def traceFamily(families):
    ...
    for family in families:
        print("\t" + family.name)
        print("\t" + "size of house :", family.size)
...
parseFamily(datadir+'config.xml')
