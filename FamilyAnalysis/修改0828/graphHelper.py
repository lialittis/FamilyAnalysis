...
def collectData(dict_df,key):
    ...
    ts = []
    for i in range(len(df['time'])):
        ts.append(df.iloc[i]['time'])
        xs.append(df.iloc[i]['x值'])
        ys.append(df.iloc[i]['y值'])
    c={ "time":ts,"x":xs,'y':ys}
    data= pd.DataFrame(c)
    #print("After Collect Data:\n",data)
    return data
...
def WashData(data):
    ...
    for idx,row in data.iterrows():
        if (row['label']==-1):
            del_list.append(idx)
      # 第1户（父母家，1m网格情况）
        if (row['label']==77)or(row['label']==107):
            del_list.append(idx)
      # 第1户（父母家，0.5m网格情况）
#         if (row['label']==303)or(row['label']==424):
#             del_list.append(idx)
      # 第2户（舅舅家，1m网格情况）
        # if (row['label']==55):
        #     del_list.append(idx)
      # 第2户（舅舅家，0.5m网格情况）
        ...
        #     del_list.append(idx)        
      # 第3户（姨妈家，1m网格情况）
        # if (row['label']==37)or(row['label']==38)or(row['label']==39)or(row['label']==49)or(row['label']==50)or(row['label']==51)or(row['label']==149):        # 删去出门对应的网格中所有点数 第1户：（1m-67) 第2户：（0.5m-221；1m-55） 第3户：（0.5m；1m-）
        #     del_list.append(idx)
      # 第3户（姨妈家，0.5m网格情况）
        # if (row['label']==170)or(row['label']==171)or(row['label']==172)or(row['label']==173)or(row['label']==174)or(row['label']==175)or(row['label']==176)or(row['label']==194)or(row['label']==195)or(row['label']==196)or(row['label']==197)or(row['label']==198)or(row['label']==199)or(row['label']==200)or(row['label']==218)or(row['label']==219)or(row['label']==220)or(row['label']==221)or(row['label']==222)or(row['label']==223)or(row['label']==224)or(row['label']==241)or(row['label']==242)or(row['label']==243)or(row['label']==244)or(row['label']==245)or(row['label']==246)or(row['label']==247)or(row['label']==585):        # 删去出门对应的网格中所有点数 第1户：（1m-67) 第2户：（0.5m-221；1m-55） 第3户：（0.5m；1m-）
        #     del_list.append(idx)            
      # 第4户（曾云一家，0.5m网格情况）
        # if (row['label']==373)or(row['label']==374)or(row['label']==397)or(row['label']==351):
        #     del_list.append(idx)
    for i in del_list:
        data=data.drop(i)
...
def createGrid(data,figName,column_num,row_num,max_x,min_x,max_y,min_y):
    ...
    #print(data)
    washed_data = WashData(data)
    
    # count values
    groups = washed_data['label'].value_counts() # 统计网格中的点数
    #print(groups)
    # groups[37]=0   # 删去门口对应的网格中所有点数 第1户：（1m-67) 第2户：（0.5m-221；1m-55）
    # groups[38]=0
    # groups[39]=0
    # groups[49]=0
    # groups[50]=0
    ...
    return washed_data,groups,grids
...
def sliceByTime(time_list,epsilon):
    ...
    for i in range(1,len(time_list)):
        ...
        if time_delta.total_seconds()/60 >= epsilon or t2.day != t1.day : #epsilon: # 时间差大于设定值，即在此期间离开该zone，停止计时
            ...
            temp_count = 1
            end_time = t1+timedelta(minutes=1) # 加1的原因是用完当前这一分钟
...
def createHeatmapByTime(dict_zones,figName,map_img,res_path,row_num,column_num,unit,max_x,min_x,max_y,min_y,groups):
    ...
    for zone in dict_zones.keys():
        for label in dict_zones[zone]:
            ...
            new_grids[r,c] = math.sqrt(groups[label])/math.sqrt(max(groups)) # /25 Do a sqrt to make the map clearer !!!
            zone_grids[r,c] = zone
    #specify size of heatmap
    fig, ax = plt.subplots(figsize=(max_x-min_x,max_y-min_y))
    ax = sns.heatmap(new_grids, cmap='Blues',linewidths=0,alpha=0.7,xticklabels =False,square = True,annot=zone_grids,yticklabels =False,mask=(new_grids==0.),center=0.5)
    ax.invert_yaxis()
    #plt.show()
    plt.imshow(map_img,zorder = 0, extent=[-0.1,(max_x-min_x)/unit-0.2,-0.1,(max_y-min_y)/unit-0.3] ) # extent=[x_left/unit,x_right/unit,y_down/unit,y_up/unit])
...
def createHeatmapByTime2(new_grids,figName,map_img,res_path,unit,max_x,min_x,max_y,min_y):
    fig, ax = plt.subplots(figsize=(max_x-min_x,max_y-min_y))
    ax = sns.heatmap(new_grids, cmap='Reds',linewidths=0,alpha=0.7,xticklabels =False,square = True,annot=False,yticklabels =False,mask=(new_grids==0.),center=0.5)
    ax.invert_yaxis()
    #plt.show()
...
def storeTimeLine(timeline,filename):
    ...
    myHelper.saveDFtoWB(df,filename)
...
def get_K(x,y,key,map_img,res_path,max_x,min_x,max_y,min_y):
    ...
    #住户1 （父母家） tag0
    if (key == 'All') or (key == 'long') or (key == 'mid'):  #针对第1户
     return 4   #tag0=4；tag1=4
    if key == 'short':  #针对第1户
     return 4   #tag0=4；tag1=4
...
def get_dict_zones(choice_data,k): #(数据，聚类数)
    ...
    # for idx,row in choice_data.iterrows():
    #     if row['label'] not in dict_zones[row['zone']]:
    #         dict_zones[row['zone']].append(row['label'])
...
def FinalAnalysis(data,key,min_count,number_cluster,epsilon,row_num,column_num,map_img,res_path,unit,max_x,min_x,max_y,min_y,groups,family_name,withTimeLine=0):
    ...
    # create files for different data
    writer = pd.ExcelWriter(datadir+family_name+key+'_choice_data.xlsx')
    # write dataframe to excel
    choice_data.to_excel(writer)
    # save the excel
    writer.save()
